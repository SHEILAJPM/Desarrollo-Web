...
class AsistenciaServiceTest {
    ...
    void marcarConDocumentoInexistenteLanzaExcepcion() {
        assertThrows(NoSuchElementException.class,
                () -> asistenciaService.marcar("99999999", TipoMarcacion.ENTRADA, -12.0464, -77.0428));
    }

    @Test
    void marcarConDocumentoValidoRegistraLaMarcacionLigadaAlTrabajador() {
        Marcacion registrada = asistenciaService.marcar(
                trabajador.getDocumentoIdentidad(), TipoMarcacion.ENTRADA, -12.0464, -77.0428);
    ...
    void listarPorTrabajadorDevuelveSoloSusMarcaciones() {
        asistenciaService.marcar(trabajador.getDocumentoIdentidad(), TipoMarcacion.ENTRADA, -12.0464, -77.0428);
        asistenciaService.marcar(trabajador.getDocumentoIdentidad(), TipoMarcacion.SALIDA, -12.0464, -77.0428);

        List<Marcacion> marcaciones = asistenciaService.listarPorTrabajador(trabajador.getId());
